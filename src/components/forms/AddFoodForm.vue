<template>
  <div>
    ...
    <b-form @submit="onSubmit" @reset="onReset" v-if="show">
      ...
      <b-form-timepicker id="timepicker-placeholder" v-model="form.time" placeholder="Choose a time" locale="es"></b-form-timepicker>
      <p>Value: '{{ form.time }}'</p>

      <b-form-group> 
      <b-form-select v-model="form.type" :options="types"></b-form-select>
      <div class="mt-3">Selected: <strong>{{ form.type }}</strong></div>
      </b-form-group>
...
export default {
  ...
  computed: {
    ...
    userFood (){
      return this.$store.state.userFoods.find(food => food.id == this.toEditID)
    }
  },
  methods: {
    onSubmit(event) {
